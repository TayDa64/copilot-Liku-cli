...
const GITHUB_WORKFLOW_PERMISSIONS_INSPECT_SCHEMA_VERSION = 'github.workflow-permissions-inspect.v1';

function buildUsageMessage() {
  return 'Usage: liku github workflow permissions inspect <path> [--body <text> | --body-file <path>] [--slug owner/repo]';
}

function inspectGitHubWorkflowPermissions(options = {}) {
  ...
  const input = resolveWorkflowTextInput({
    ...
    requirePath: false,
    emptyBodyMessage: 'GitHub workflow permissions inspect requires non-empty workflow content.',
  });

  const report = {
    schemaVersion: GITHUB_WORKFLOW_PERMISSIONS_INSPECT_SCHEMA_VERSION,
    success: true,
    featureFlagEnabled,
...
module.exports = {
  GITHUB_WORKFLOW_PERMISSIONS_INSPECT_SCHEMA_VERSION,
  buildUsageMessage,
  inspectGitHubWorkflowPermissions,
};
