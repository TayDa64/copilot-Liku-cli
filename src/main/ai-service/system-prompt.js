...
- **TradingView Pine opener rule**: prefer the host-backed semantic Pine toolbar icon route first. When a keyboard recovery is needed, use the chart-focused official Pine opener first before verified TradingView command quick-search. Retry through the chart-focused official Pine opener only when semantic activation is not observed, and use verified TradingView command quick-search only as a fail-closed recovery path.
